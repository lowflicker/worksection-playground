...
За замовчуванням — інтеграції Worksection (Google Drive / Docs / Sheets / Slides, Slack, Telegram, Viber, Gmail, Google Calendar, Apple iCal, Zapier, Make). У плейграунді група «Логотипи» вмикає й вимикає марки з каталогу, а в поле можна вписати slug із simpleicons.org або URL до свого SVG; сніпет перелічує плитки в тому ж порядку. Бренд-марки в плейграунді та `demo.html` беруться з CDN Simple Icons (CC0) і Devicon (MIT), тож без інтернету плитки порожні. На сайті поклади свої SVG у `logos/` і вкажи їх у `src`. Фон плитки — `--iw-surface`, розмір логотипа — опція `logo`. Короткий список не повторюється смугами: рядки йдуть по списку з кроком золотого перетину.
