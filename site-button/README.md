...
Кнопки сайту — джерело правди про те, що є в коді. Система `.btn` тут не переписана: плейграунд і демо вантажать [`site-css/buttons.css`](../site-css/buttons.css) — це `assets/css/buttons.css` з worksection.com дослівно. Панель пропонує рівно ті класи, що в ньому є, аркуш під кнопкою показує кожен варіант у кожному розмірі, сніпет — розмітку сайту.
