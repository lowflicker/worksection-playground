...
На сайт беруть із цього файлу лише блок `.btn.btn-beam` (кінець файлу) і файли ефекту `beam.css` + `beam.js` з `../border-beam/`. Плейграунд: `../index.html`, вкладка «Button». Кнопка — компонент: її копії стоять у «Hero» і повторюють налаштування з цієї вкладки.
