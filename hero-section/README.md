...
Поява (`entrance`), як на sketch.com, лише в широкій композиції (≥ 640 px): на телефонах ні початкового стану, ні спостерігача, ні скрол-нахилу немає, блок одразу в кінцевому стані. Перший раз, коли шоста частина блоку з вкладками і сценою з’являється у в’юпорті (IntersectionObserver), вкладки й стрілки по черзі (45 мс) піднімаються з нуля прозорості, сцена за 0.8 с проявляється з нахилу `tiltIn` (`rotateX`, перспектива на `.hero__view`, вісь по центру, тож низ стартує ближче до глядача) і осідає, а телефон на 250 мс пізніше вилітає з `fly` px справа з нуля прозорості. Це WAAPI на зареєстрованих властивостях `--hero-lean` і `--hero-fly-x`; зсув телефона вкладений у його базовий `translate`, тому поява не конфліктує зі свапом. До появи все це в початковому стані лише якщо `hero.js` встиг додати `hero--entrance`, тож без JS нічого не ховається.

Lighthouse (заміри на demo.html, Lighthouse 13, локально): mobile 100, FCP 0.9 с, LCP 1.58 с, TBT 0, CLS 0, SI 0.9 с; desktop 100, FCP 0.24 с, LCP 0.38 с, SI 0.31 с. Перший екран на телефоні: 33 + 30 KB картинок (AVIF), ~5 KB CSS і ~3 KB JS у gzip. Сцена стартує з `fadeFrom` = 80 % прозорості, а не з нуля: десктопний скріншот це LCP-елемент, а елемент з `opacity: 0` Chrome не рахує намальованим, тож старт з нуля відсунув би LCP на весь час появи. Вкладки й стрілки маленькі, вони стартують з нуля. Рухається лише `opacity` і transform-властивості, тому CLS не змінюється; скрипт той самий, TBT не росте. Залишковий нахил `tiltRest` сходить до нуля при скролі через `animation-timeline: view()`; де таймлайнів немає, його просто нема. `hero.enter()` програє появу ще раз.
