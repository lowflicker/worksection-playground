...
Lighthouse. Сцена стартує з `fadeFrom` = 50 % прозорості, а не з нуля: десктопний скріншот це LCP-елемент, а елемент з `opacity: 0` Chrome не рахує намальованим, тож старт з нуля відсунув би LCP на весь час появи. Вкладки й стрілки маленькі, вони стартують з нуля. Рухається лише `opacity` і transform-властивості, тому CLS не змінюється; скрипт той самий, TBT не росте. Залишковий нахил `tiltRest` сходить до нуля при скролі через `animation-timeline: view()`; де таймлайнів немає, його просто нема. `hero.enter()` програє появу ще раз.
...
const hero = new Hero('#hero', {
  ...
  fadeFrom: 0.5,       // з якої прозорості стартує сцена; не 0, інакше LCP-скріншот рахується ненамальованим
