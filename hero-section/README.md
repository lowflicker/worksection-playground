...
Поява при скролі (`entrance`). Сцена зі скріншотами стартує нахиленою на `--hero-tilt-in` (перспектива + `rotateX`, вісь по верхньому краю, тож низ ближче до глядача) і випрямляється в міру входу у в’юпорт; телефон вилітає справа з нуля прозорості, трохи пізніше. Це scroll-driven animations (`animation-timeline: view()` з іменованою `view-timeline` на сцені), без JS і без скрол-лісенерів. Зсув телефона йде через зареєстровану властивість `--hero-fly-x`, вкладену в його базовий `translate`, тому поява не конфліктує зі свапом. Safari 26+, Chrome 115+; де таймлайнів немає, видно одразу кінцевий стан.
...
const hero = new Hero('#hero', {
  ...
  entrance: true,      // поява при скролі: скріни випрямляються з нахилу, телефон вилітає справа
  tiltIn: 14,          // deg, початковий нахил (низ ближче до глядача)
