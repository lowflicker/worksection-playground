...
Зміна вкладки: стара картинка лишається клоном поверх нової і тане, нова проявляється під нею (`switch: 'fade'`). У режимі `slide` вони ще й зсуваються на 6 % у бік гортання, у `zoom` стара осідає до масштабу .97, нова з нього виростає. Клон додає `hero.js`, розмітка лишається з одним `<picture>` на скрін. Напрямок гортання (`--hero-dir`) використовує підпис вкладки на телефоні.
...
const hero = new Hero('#hero', {
  ...
  switch: 'fade',      // 'fade' | 'slide' | 'zoom': чистий кросфейд, зсув у бік гортання, або осідання з масштабом
});
