...
- кожна вкладка несе свою пару скріншотів у `data-desktop` / `data-phone`. Вкладка без них лишає поточні картинки;
...
- z-index міняється по закінченні переходу (перехід на `z-index` з нульовою тривалістю і затримкою), тож той, що росте, весь час зверху;
- бейдж-підказка на мініатюрі контр-масштабується (`scale(1 / s)`), тому має постійний розмір.
...
.hero {
  ...
  /* рух (перші три пише hero.js з опцій) */
  --hero-swap: 600ms;
  ...
  --hero-desk-s: .45;     --hero-desk-tx: 5cqw;  --hero-desk-ty: 76.5cqw;
  --hero-phone-w: 52cqw;  --hero-phone-x: 42cqw; --hero-phone-y: 0cqw;
  --hero-phone-s: .685;   --hero-phone-tx: 59cqw; --hero-phone-ty: 35cqw;
}
```

Значення за замовчуванням повторюють мобільний макет Figma (402 px): десктоп 560 px завширшки з відступом 20, телефон 143 px у правому нижньому куті. Композицію зручно підбирати в плейграунді, група «Композиція».

## Що ще варто знати

- Скріншоти в `img/` це WebP з Figma у 2x/3x. Для сайту логічно додати `srcset` або `<picture>`; `hero.js` міняє лише `src`, тож для `srcset` доведеться додати ще один `data-*`.
