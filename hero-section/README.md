...
На сайт беруть `hero.css` + `hero.js` + власну розмітку і скріншоти. Кнопки CTA — компонент сайту `.btn` (він на сайті вже є; у плейграунді його дзеркало в `../site-button/`). Промінь на головній кнопці — `.btn-beam` з `site-button/button.css` плюс `beam.css` + `beam.js` з `../border-beam/`. Плейграунд: `../index.html`, вкладка «Hero».
