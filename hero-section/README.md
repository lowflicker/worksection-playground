...
Поява (`entrance`), як на sketch.com. Перший раз, коли п’ята частина сцени з’являється у в’юпорті (IntersectionObserver), сцена за 0.8 с проявляється з нахилу `tiltIn` (`rotateX`, перспектива на `.hero__view`, вісь по центру, тож низ стартує ближче до глядача) і осідає, а телефон на 250 мс пізніше вилітає з `fly` px справа з нуля прозорості. Це WAAPI на зареєстрованих властивостях `--hero-lean` і `--hero-fly-x`; зсув телефона вкладений у його базовий `translate`, тому поява не конфліктує зі свапом. До появи сцена прихована лише якщо `hero.js` встиг додати `hero--entrance`, тож без JS нічого не ховається. Залишковий нахил `tiltRest` сходить до нуля при скролі через `animation-timeline: view()`; де таймлайнів немає, його просто нема. `hero.enter()` програє появу ще раз.
